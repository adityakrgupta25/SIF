...
def get_batches(file, words, batch_size=50000, current_count=0):
    we = []
    batch = 0
    done = False
    for line in file:
...
def getWordmap(textfile):
    ...
    with open(textfile, 'r') as f:
        ...
        words[line[0]] = n
        n += 1
        We = np.array([v], dtype=float)

        batch_size = 50000
        done = False
        while not done:
            arr, done, n = get_batches(f, words, batch_size=50000, current_count=n)
            We = np.append(We, arr, axis=0)
            print("read until:", n, "done:", done)
